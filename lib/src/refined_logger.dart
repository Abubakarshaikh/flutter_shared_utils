...
const String _loggerKey = 'logger';

/// A logger used to log errors in the root zone.
final DefaultLogger _logger = DefaultLogger();
...
class DefaultLogger extends RefinedLogger {
  ...
  final StreamController<LogWrapper> _controller =
      StreamController<LogWrapper>();
  late final Stream<LogWrapper> _logWrapStream =
      _controller.stream.asBroadcastStream();
  late final Stream<LogMessage> _logStream =
      _logWrapStream.map((LogWrapper wrapper) => wrapper.message);
  ...
  void _init(LoggingOptions options) {
    ...
    _logWrapStream.listen((LogWrapper l) => _printLogMessage(l, options));
  }
  ...
  void _printLogMessage(LogWrapper wrappedMessage, LoggingOptions options) {
    ...
    final String formattedMessage = options.formatter != null
  ...
  String _defaultFormatter(LogWrapper wrappedMessage, LoggingOptions options) {
    final LogMessage message = wrappedMessage.message;
    final String time =
        options.showTime ? message.timestamp.toIso8601String() : '';
    final String emoji = options.showEmoji ? message.level.emoji : '';
    final LogLevel level = message.level;
    final String content = message.message;
    final StackTrace? stackTrace = message.stackTrace;
...
extension on LogLevel {
  ...
  String get emoji => const <LogLevel, String>{
        LogLevel.trace: '🔍',
        LogLevel.debug: '🐛',
        LogLevel.info: 'ℹ️',
        LogLevel.warn: '⚠️',
